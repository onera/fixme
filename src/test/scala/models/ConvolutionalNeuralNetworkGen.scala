...
object ConvolutionalNeuralNetworkGen {
  ...
  /**
   * Define random shapes
   */
  val shape2DGen: Gen[Shape2D] = for {
    x <- choose(1, 124)
    y <- choose(1, 124)
    c <- choose(1, 1000)
  } yield Shape2D(x, y, c)
  ...
  /**
   * Define random convolution layers
   */
  val conv2DGen: Gen[Convolution2D] = for {
    input <- shape2DGen
    kernel_size <- shape2DGen
    strides <- shape2DGen
    filters <- choose(1, 1000)
    name <- alphaStr
  } yield
    Convolution2D(name, input, strides, kernel_size, valid_padding = true, filters)

  /**
   * Define random pooling layers
   */
  val pool2DGen: Gen[Pooling2D] = for {
    input <- shape2DGen
    kernel_size <- shape2DGen
    strides <- shape2DGen
    name <- alphaStr
  } yield
    Pooling2D(name, input, strides, kernel_size, valid_padding = false)

  /**
   * Define random input layers
   */
  val input2DGen: Gen[Input2D] = for {
    input <- shape2DGen
    output <- shape2DGen
    name <- alphaStr
  } yield
    Input2D(name, input, output)

  /**
   * Define random dense layers
   */
  val dense2DGen: Gen[Dense] = for {
    input <- shape2DGen
    output <- shape2DGen
    name <- alphaStr
  } yield
    Dense(name, input, output)

  /**
   * Define random layers
   */
  val layer2DGen: Gen[Layer2D] = frequency(1 -> conv2DGen, 1 -> pool2DGen, 1 -> dense2DGen)

  /**
   * Define random cnn constituted of conv/pool blocks and dense layers where shapes are consistent
   */
  val cnn2DGen: Gen[ConvolutionalNeuralNetwork] = for {
    name <- alphaStr
    input <- input2DGen
    blocks <- listOfN(10, listOfN(3, conv2DGen))
    pools <- listOfN(3, pool2DGen)
    dense <- listOfN(3, dense2DGen)
    layers = input +: (blocks.zip(pools).flatMap(p => p._1 :+ p._2) ++ dense)
  } yield {
    val cnn = layers.foldLeft(List.empty[Layer2D])({
      case (Nil, l) => l :: Nil
      case (acc, c: Convolution2D) => acc :+ c.copy(inputShape = acc.last.outputShape)
      case (acc, c: Pooling2D) => acc :+ c.copy(inputShape = acc.last.outputShape)
      case (acc, c: Dense) => acc :+ c.copy(inputShape = acc.last.outputShape)
      case (acc, l) => acc :+ l
    }).tail
    ConvolutionalNeuralNetwork(input, cnn, name)
  }
